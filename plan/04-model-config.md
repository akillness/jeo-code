...
provider; local and OpenAI-compatible endpoints work keylessly; `jeo setup`/`jeo doctor`
...
- **jeo** decision: keep the prefix router (simple, predictable) and add a thin **alias + registry** layer
...
- Optional `jeo models` command: list configured + locally-discovered models (`/api/tags`, `/v1/models`).
...
- **Slice 2 — `jeo models` command** (`src/commands/models.ts` + `runner.ts` entry): list/probe. → `executor`.
...
- [ ] `jeo models` lists configured aliases + probed local models; exits 0 when ollama is up.
...
jeo models                                  # lists aliases + local models
JEO_CONFIG_DIR=/tmp/c jeo doctor            # [READY] for the configured default
```
