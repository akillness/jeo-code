...
description: >
  ...
  `jeoc autopilot` CLI. Triggers on: autopilot, jeoc autopilot, ratchet build,
  keep-or-revert loop, frozen eval autopilot.
allowed-tools: Read Write Bash
metadata:
  tags: autopilot, autoresearch, ratchet, jeoc, jeo-code, keep-or-revert, frozen-eval
...
`jeoc autopilot` is the autopilot end-to-end build loop **plus** autoresearch's
...
Branding: binary `jeoc`, project `jeo-code`, state under `.jeoc/` (rebrand of
`gjc` / `gajae-code` / `.gjc`).
...
5. **Append-only log.** `.jeoc/autopilot/log.jsonl` keeps reverted attempts as evidence.
...
jeoc autopilot init --task "make tests green" --eval "bash eval.sh" --goal min \
...
jeoc autopilot step --change "use index for lookup" --on-revert "git reset --hard HEAD~1"
...
- `jeoc autopilot`: ratchet generalized to any build task with a frozen evaluator.
