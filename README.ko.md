...
상태 줄은 매 틱 바뀌는 장식 문구 대신 **지금 실제로 하는 일**(진행 중인 파일·명령, 활성 plan 단계, plan 진행도, 레이트리밋 백오프 중에는 `rate limited (HTTP 429) — auto-retry #2 in 4s` 카운트다운)을 현재 step 경과와 함께 보여줍니다. 인라인 턴에서 진화 정체성은 마지막 `Evolved to: …` 요약 한 줄로만 남고, ASCII 아트 헤더는 레거시 `JOC_TUI_ALT_SCREEN=1` 박스 모드에 유지됩니다. `task`로 위임된 **서브에이전트의 진행 상황**(할당·`step N/M`·중첩 툴 호출의 실제 대상 `read src/x.ts`·`bash: …`·결과 요약)도 gjc처럼 스트림에 실시간 표시됩니다.
