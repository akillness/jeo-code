# 04 — autopilot × autoresearch (jeo CLI)

`jeo autopilot` 분기 설계. `/skill:autopilot`(end-to-end 빌드 루프)에 `/skill:autoresearch`의
래칫 규율을 이식해 정밀하게 작성한 결과다. 동작하는 CLI(`bin/jeo.ts` + `src/autopilot.ts`)로 구현.
...
plain autopilot은 "QA 통과 → 완료"였다. jeo autopilot은 "각 변경이 **고정된 점수**를 개선했음을
...
5. **append-only 로그** — `.jeo/autopilot/log.jsonl`. revert된 시도도 증거로 보존.
...
```sh
jeo autopilot init --task <t> --eval <cmd> [--goal min|max|gate] [--timeout S] [--patience N]
jeo autopilot baseline
jeo autopilot step --change <desc> [--on-revert <cmd>]
jeo autopilot loop --runner <cmd> [--max N] [--on-revert <cmd>]
jeo autopilot status [--json]
```

- 구현: `bin/jeo.ts`(umbrella) → `src/autopilot.ts`(`runAutopilot`). 외부 의존성 0(Node stdlib).
- 상태: `.jeo/autopilot/{session.json, log.jsonl}`.
...
| 리브랜드: `jeo ledger`가 `.jeo/ledger.jsonl`에서 verdict=verified | ✅ |

## 7. ledger와의 관계

`jeo ledger`(크로스-plan 원장)와 `jeo autopilot`(빌드 래칫)은 같은 `jeo` CLI의 두 그룹.
