...
> The living roadmap for `joc`'s functional surface: the agentic loop, the
...
## 1. Goal
Keep `joc` at gjc's workflow parity (clarify → plan → execute → verify) while
...
- gjc: 26 commands incl. session/worktree/stats/web-search — joc keeps a tight core (launch + 4 pipeline +
...
- joc decision: deepen the existing core; do **not** chase gjc's full command count.
...
- **Slice C — `joc resume` top-level**: lift session resume out of `launch` into a first-class command. → `executor`.
...
- [ ] `joc launch --max-steps 5` stops at 5 steps (asserted in a unit test against a mock that never calls done).
