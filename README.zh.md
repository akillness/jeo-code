...
- **并行子代理、有界异步委派与监视器** — `task {tasks:["task 1","task 2"]}` 会在 `MAX_FANOUT` 上限内并发运行相互独立的任务(执行器和只读角色都支持，结果保持输入顺序)；`task {detached:true}` 最多运行四个具有稳定 ID、可独立取消的子代理，并提供生命周期控制和 `irc` 引导。`monitor {command, kind, description, timeout?, persistent?}` 将有界 stdout 观测作为明确非指令性的后台数据送入回合；TUI 和普通日志会分别显示每个运行中的工作者。
