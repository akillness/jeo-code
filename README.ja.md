...
- **並列サブエージェント・境界付き非同期委任とモニター** — `task {tasks:["task 1","task 2"]}` は独立した割り当てを `MAX_FANOUT` の範囲で同時実行し(Executor/読み取り専用ロールに対応、結果順序は入力順を維持)、`task {detached:true}` は安定した ID を持つ個別にキャンセル可能なサブエージェントを最大4つ実行してライフサイクル制御と `irc` ステアリングを提供します。`monitor {command, kind, description, timeout?, persistent?}` は境界付き stdout 観測を明示的に命令ではないバックグラウンドデータとしてターンへ渡し、TUI と通常ログは実行中の各ワーカーを区別して表示します。
