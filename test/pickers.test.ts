...
test("buildModelChoices lists ready providers first and groups them", () => {
  ...
  // first group should be a ready provider (openai or ollama)
  expect(["openai", "ollama"]).toContain(choices[0]!.group);
  ...
  expect(choices.some(c => c.group === "anthropic (no credential)")).toBe(true);
});
...
test("modelPicker excludes unready providers when includeUnready:false", () => {
  ...
  expect(list.visible().every(i => i.group === "openai")).toBe(true);
});
