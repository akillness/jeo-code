...
## 사이클 렛저 (라운드 15 — 중간 판단 가드 분류·추출)
- cycle 25 (2026-06-14): **gjc식 분류 적용** — gjc는 중간 판단을 `gjc-runtime/ultragoal-guard.ts`의 명명된 `UltragoalGuardState` 판별 유니온으로 분리하지만, jeo는 동일 로직을 `engine.ts` while 루프에 인라인 불리언·문자열로 흩어 둠. 신설 `src/agent/loop-guards.ts`가 그 분류를 계승: 명명 택소노미 `GuardState`(repeat/cycle/consecutive_failure/invalid_tool/parse_salvage/context_overflow/refusal/done_unverified/done_hook_failing/done_ok), 단일 출처 `GUARD_LIMITS`(MAX_REPEAT 등 6개 임계), 순수 분류자 `isVerificationSignal`/`repeatHint`/`classifyDoneGate`(B4 done 검증 게이트의 직계 후손 — 변이 후 검증 없음 또는 훅 실패 시 1회 차단 평결, escape hatch는 호출부 latch 소유). engine.ts는 제어흐름(history 변이·step++·return)만 보유하고 판단은 모듈에 위임 — **동작 불변**. 신규 테스트 9종(loop-guards). full 1687 pass / 0 fail, typecheck 0.
- cycle 25 후속 (2026-06-14, ponytail): **죽은 병렬 모듈 `src/agent/tool-registry.ts` 삭제** — V2 리팩터(2026-06-10) 잔재로, 어떤 `.ts`도 import하지 않는 고아였다. 그 5개 export(`ToolHandler`/`DEFAULT_TOOLS`/`TOOL_PROTOCOL`/`READONLY_TOOL_PROTOCOL`/`nearestToolName`)는 전부 engine.ts가 더 풍부한 라이브 버전으로 재정의·사용 중(tool-registry 사본은 mkdir/delete/web_search·onProgress 누락, `nearestToolName`은 실 edit-distance 대신 `10` 스텁이라 퍼지 매칭 불발). 한 함수만 통합하는 대신 모듈 전체를 제거하는 것이 진짜 합치. AGENTS.md Key Files에서도 행 삭제. 추가로 loop-guards.ts의 장식적이던 `GuardState`(어디서도 미참조)를 `DoneGateVerdict.state = Extract<GuardState, …>`로 배선해 택소노미를 단일 출처로 승격(verdict 리터럴 유니온 중복 제거 → 드리프트 불가). full 1687 pass / 0 fail, typecheck 0.

## 합의 라운드 16 (현행 gjc 시스템 프롬프트 재diff, 2026-06-19)
**축**: v0.4.3 분석(라운드 1~15) 이후의 **현행 gjc** 표면 재대조. 프롬프트/도구출력/워크플로/플래닝/프로바이더 패리티는 사실상 소진 — 잔여 gjc 델타는 무거운 아키텍처(비동기 서브에이전트·irc) 아니면 기도입/기탈락. 유일하게 남은 경량·고가치 항목은 gjc `<verification>` 블록의 **테스트 품질 계약**(jeo의 검증 디렉티브는 "테스트를 돌려라"만 말하고 *무엇이 좋은 테스트인지*는 침묵 — 마라톤 자신의 "신규 테스트 N종" 품질에 직접 영향). 동시에 그 검증 self-check 문장이 `engine.ts`(executorSystemPrompt 기본인자)와 `launch.ts`(인터랙티브 프롬프트)에 **verbatim 중복**된 parallel-convention 스멜 발견.
- **cycle 26 (2026-06-19)**: gjc `<verification>` 테스트 품질 계약 계승 + 중복 제거. 신설 `VERIFICATION_DIRECTIVE` 단일 export(engine.ts) = 기존 done self-check + "테스트는 관측가능 동작·경계값·분기조건·불변식·에러처리를 검증하고 기본값/항진명제는 단언하지 말 것"(pi-mono <300토큰 유지). executorSystemPrompt 기본인자가 이를 참조, launch.ts가 하드코딩 문자열 대신 import 소비(두 갈래 드리프트 불가능한 단일 출처화), executor 서브에이전트 프롬프트(executor.md)에도 동일 제약 1줄. 신규 lock 테스트 2종(prompt-discipline). typecheck 0, full 1709 pass / 0 fail(+1 사전존재 tui truecolor 플레이크 — git stash로 본 변경 무관 확인).
- **라운드 16 종료** — 누적 26사이클. 현행 gjc 재diff 결과 경량 백로그 소진 확인. 미도입 잔여는 전부 무거운 아키텍처 항목(비동기/detached 서브에이전트 + irc 피어 메시징, ast_grep/lsp = 의존성)으로 pi-mono·zero-dep 노선상 비도입 유지.