...
- **크래시 내구·로컬 우선** — 모든 상태는 `.joc/` 아래 원자적 쓰기, 크로스 프로세스 런 락, 실패 태스크 마커 + 재개 시 부분 편집 경고.
...
요구사항 → 플랜 → 승인 → 실행 → 검증이 `.joc/state/`로 이어지며, 모든 핸드오프에 **차단 가능한 진짜 게이트**가 있습니다:
...
전역에서 한 번 활성화(`~/.joc/config.json`에 `"hooks": { "enabled": true }`)한 뒤 프로젝트별 post-edit 검사를 추가하면, 에이전트가 실패를 읽고 `done` 전에 고칩니다:
...
- 전역 설정: `~/.joc/config.json` (모델 선택은 MRU 영속)
