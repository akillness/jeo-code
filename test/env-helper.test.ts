...
test("jeoEnv reads only canonical JEO_* values", () => {
  expect(jeoEnv("TUI_THEME", { JEO_TUI_THEME: "matrix" })).toBe("matrix");
  expect(jeoEnv("TUI_THEME", {})).toBeUndefined();
  // An explicitly EMPTY JEO_* value is still a value (not || semantics).
  expect(jeoEnv("FLAG", { JEO_FLAG: "" })).toBe("");
});
