 ...
 * (gjc `RateLimitPool` parity).
 *
 * Multiple jeo sessions on one host share a single bot token and paired chat.
 ...
 *
 * Current jeo callers only actually submit to the `"finalized"` lane in
 * practice (subagent status pushes + main-session `turn_stream`/
 * `context_update` mirroring). The `"ask"` and `"live"` lanes exist in the
 * type for architectural completeness/future use — jeo has no `ask` tool
 * (Tier 3, out of scope) and no incremental token-level stream to mirror yet
 * (see the Tier 2 contract) — and `"idle"` is likewise unused today. All four
 * lanes are kept exactly as gjc has them; this is a note about CURRENT
 * callers, not a scope reduction of the module itself.
 */
...
/** Build a fixed lookup table with one entry per priority lane (lanes are a
 *  small, static, never-added-to-at-runtime key set — a plain `Record` avoids
 *  the `Map` overhead/API surface this scheduler never needs). */
function createLaneRecord<V>(factory: () => V): Record<RateLimitLane, V> {
  const record = {} as Record<RateLimitLane, V>;
  for (const lane of LANE_PRIORITY) record[lane] = factory();
  return record;
}
 ...
 *
 * Callers {@link RateLimitPool.submit} work and periodically
 * {@link RateLimitPool.drain} (e.g. on a timer or after each submit); `drain`
 * returns the items granted a send slot, in the order they should be sent.
 */
export class RateLimitPool<T = unknown> {
  ...
  private readonly lanes: Record<RateLimitLane, QueuedItem<T>[]>;
  /** Rotating session cursor per lane for round-robin fairness. */
  private readonly laneCursor: Record<RateLimitLane, number> = createLaneRecord(() => 0);
  ...
  constructor(options: RateLimitPoolOptions = {}) {
    ...
    this.lanes = createLaneRecord<QueuedItem<T>[]>(() => []);
  }

  /** Number of items currently queued across all lanes. */
  get pending(): number {
    let total = 0;
    for (const lane of LANE_PRIORITY) total += this.lanes[lane].length;
    return total;
  }
  ...
  private pickFairIndex(lane: RateLimitLane, queue: QueuedItem<T>[]): number {
    ...
    const cursor = this.laneCursor[lane];
    ...
    for (let offset = 0; offset < sessions.length; offset++) {
      ...
      if (idx >= 0) {
        this.laneCursor[lane] = (cursor + offset + 1) % sessions.length;
        return idx;
      }
    }
    return 0;
