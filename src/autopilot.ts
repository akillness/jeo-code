/**
 * jeoc autopilot — autonomous build loop hardened with autoresearch ratcheting.
 *
 * Fuses two skills:
 ...
 *
 * State (per cwd):
 *   .jeoc/autopilot/session.json   frozen contract (immutable for the session)
 *   .jeoc/autopilot/log.jsonl      append-only attempt log (baseline, steps, stops)
 *
...
const AP_DIR = path.join(".jeoc", "autopilot");
...
function die(msg: string): never {
  console.error(`jeoc autopilot: ${msg}`);
  process.exit(1);
}
...
function loadSession(): Session {
  if (!fs.existsSync(SESSION)) die("no session — run: jeoc autopilot init --task <t> --eval <cmd>");
  return JSON.parse(fs.readFileSync(SESSION, "utf8")) as Session;
}

function appendLog(ev: Omit<LogEvent, "ts">): LogEvent {
...
function cmdInit(flags: Record<string, string>): void {
  ...
  console.log(`jeoc autopilot: session frozen → ${SESSION}`);
...
function cmdBaseline(): void {
  ...
  console.log(`jeoc autopilot: baseline score=${fmt(score)} passed=${passed}`);
}

function cmdStep(flags: Record<string, string>): void {
  const s = loadSession();
  if (s.goal !== "gate" && !hasBaseline()) die("record a baseline first: jeoc autopilot baseline");
  ...
  if (decision === "revert" && flags["on-revert"]) {
    try {
      execSync(flags["on-revert"], { stdio: "inherit", shell: "/bin/bash" });
    } catch {
      console.error("jeoc autopilot: --on-revert hook failed (decision still logged)");
    }
  }
  ...
  console.log(`jeoc autopilot: step ${decision.toUpperCase()}  (${cmp})  — ${change}`);
}
...
function cmdLoop(flags: Record<string, string>): void {
  ...
  if (s.goal !== "gate" && !hasBaseline()) {
    const { score, passed, output } = runEval(s);
    appendLog({ type: "baseline", score, passed, output });
    console.log(`jeoc autopilot: auto-baseline score=${fmt(score)}`);
  }
  ...
  for (let i = 1; i <= max; i++) {
    ...
    let runnerOk = true;
    try {
      execSync(runner, { stdio: "inherit", timeout: s.timeoutSec * 1000, shell: "/bin/bash" });
    } catch {
      runnerOk = false;
    }
    if (!runnerOk) {
      appendLog({ type: "stop", reason: "runner_failed", iteration: i });
      console.log(`jeoc autopilot: stop — runner failed at iteration ${i}`);
      return;
    }
    ...
    console.log(`jeoc autopilot: loop ${i}/${max} ${decision.toUpperCase()} score=${fmt(score)} (sinceImprove=${sinceImprove})`);

    if (s.goal !== "gate" && sinceImprove >= s.patience) {
      appendLog({ type: "stop", reason: "converged", iteration: i, patience: s.patience });
      console.log(`jeoc autopilot: stop — converged (no improvement in ${s.patience} steps)`);
      return;
    }
  }
  appendLog({ type: "stop", reason: "max_iterations", iteration: max });
  console.log(`jeoc autopilot: stop — reached max ${max} iterations`);
}
...
function cmdStatus(flags: Record<string, string>): void {
  ...
  console.log("jeoc autopilot status\n");
...
export function runAutopilot(argv: string[]): void {
  ...
  switch (cmd) {
    ...
    default: die(`unknown subcommand: ${cmd} (try: jeoc autopilot help)`);
  }
}
