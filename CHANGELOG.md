...
- `bun run typecheck` clean.
- Full `bun test`: this host ran under sustained, unrelated multi-tenant load throughout this pass (load average 21-35, 29 concurrent users) — full-suite runs surfaced 7-10 timing-shaped failures per run in areas completely unrelated to this change (model-pin persistence, route-fallback timing, browser automation, LSP), none reproducing in isolation; every file actually touched this pass stayed green and fast in repeated isolated runs regardless of host load. Recorded honestly rather than claiming a clean full-suite number this host could not currently produce.
