...
요구사항을 먼저 정리하고 계획, 실행, 검증까지 진행할 때 사용합니다. 단계는 상태(`.joc/state/`)로 이어지며 게이트가 있습니다: deep-interview가 **시드를 동결**(ambiguity ≤ 20%; `--auto`는 최선의 시드를 항상 동결)해야 MutationGuard가 코드 수정을 허용하고 ralplan이 진행됩니다 → ralplan은 **승인 대기** 플랜을 만들고 → `joc approve <plan>`로 승인해야 → team이 실행하며 → ultragoal이 team 실행을 검증합니다.
