...
## Pass 2026-06-25 — gjc README parity (3 sections) + team / memory / speed / workflow re-diagnosis

### A. gjc README sections analyzed (https://github.com/Yeachan-Heo/gajae-code)
gjc README §190 "Skill migration and bundled skill inspection", §202 "Works beside your existing agent or bot", §248 "Development". jeo already had the first and third (added 0.7.17). This pass focused on **"Works beside"**, which existed in jeo as a thin 5-row table that ignored two real jeo capabilities.

**Applied (this pass):** enriched the "Works beside" table in `README.md` + `README.ko.md` / `README.ja.md` / `README.zh.md` to reflect jeo's actual surface, verified against source:
- `--worktree <name>` — `src/commands/launch/tmux.ts:253 resolveWorktree`: reuses an existing path, else `git worktree add -b <basename>` (falls back to attaching an existing branch). Routed via `src/cli/runner.ts` (`VALUE_FLAGS`) and `src/commands/launch.ts:320`. Now shown for the Codex/Claude/Claw rows.
- `jeo mcp serve` / `jeo mcp tools` — `src/commands/mcp.ts:4-16` (MCP stdio server). This is jeo's real "external controller / bot" contract; the row no longer says "standard terminal/CLI interfaces" (scrollback scraping) but "MCP tool contract over stdio".
- Kept the `-q`/`--quiet` (`JEO_QUIET=1`) + `-p`/`--print` quiet guidance.

### B. `jeo team` re-analysis (src/commands/team.ts, evidence-cited)
Robust, gate-heavy serial executor. Confirmed safeguards:
- **Approval + consensus gates** — refuses unless `ralplan` state `current_phase==="complete"` AND `approved` (team.ts ~206-216), and verifies `sha256(planContent) === planState.consensus_hash` so a plan edited after critic review is rejected (Round-13, team.ts ~248).
- **Cross-process run lock** — `acquireWorkflowRunLock("team")` prevents two runs double-popping `pending_tasks` (Round-8).
- **Stale-plan reset** — a leftover `team-state.json` from a different `plan_path`/`slug` reinitializes execution instead of false-completing (Round-7).
- **Corrupt-state refusal** — `readWorkflowStateStrict` throwing → hard error, never a silent restart that re-runs completed tasks.
- **Failed-task durability** — on failure persists `current_phase:"failed"` + `failed_task`; next run probes `gitDirtyCount` and reports the concrete uncommitted count before re-running on possible partial edits (Round-8/12).
- **Mutation audit** — counts successful `write|edit|mkdir|delete` (`fileMutations`) apart from `bash` (`bashRuns`); a non-readOnly role finishing with `fileMutations===0` is flagged; `--strict-mutations` hard-fails only when `bashRuns===0` too (bash-only stays advisory) — Round-8/11/12.
- **Role gate** — `parseRoleGateVerdict`: architect needs `Architectural Status ∈ {CLEAR,WATCH,BLOCK}` + `Code Review Recommendation ∈ {APPROVE,COMMENT,REQUEST CHANGES}` (BLOCK / REQUEST CHANGES gate); critic is **fail-closed** — only a literal `[OKAY]` first line passes.
- **Bounded delegation** — subagents run with `budget:{maxExtensions:0}` (exact step contract; orchestrator owns retries), per-step `maybeCompact`.

**Honest gaps (not bugs):** (1) strictly serial — one failing task halts the whole run, no continue-on-error / parallel fan-out; (2) the mutation audit counts tool-call *success*, not real diff content, so a no-op edit still reads as a mutation (acknowledged heuristic); (3) bash-only mutating runs can never hard-fail even under `--strict-mutations`.

### C. Memory / speed / workflow re-diagnosis (src/agent/memory*.ts)
- **Injection safety** — `frameMemory()` hard-caps at `MEMORY_INJECT_MAX_CHARS=5000`, neutralizes `<project_memory>` fence tags, and frames as DATA (shared by bundle + legacy path so neither bypasses hardening). Distilled-from-transcript text is treated as untrusted. ✅
- **Budget + relevance** — `memoryPromptSection` → `selectWithinBudget` greedily keeps whole concepts in `priorityOrder` (high-confidence core → query score → 1-hop graph neighbours → input order). No mid-concept truncation. Query seeds from the one-shot task text. ✅
- **Parse cache (speed)** — `parsedConceptCache` keyed on `mtimeMs:size`, LRU, cap 512: an unchanged file skips read+parse on the per-subagent-spawn `loadConcepts`. ✅
- **Detached distill, orphan-safe** — exit spawns a detached `jeo memory-distill <payload>` (`node:child_process` `detached:true`, unref'd) so quitting is instant; `runMemoryDistillCommand` **always `exit(0)` in `finally`** (even on the 20s `Promise.race` timeout) — this is the fix for the "jeo/bun process pileup, RSS creep" report, because the race only rejects and never aborts the fetch socket. Stale `pending-distill-*.json` are GC'd after 24h; idle jeo tmux REPLs are reaped (6h TTL). ✅
- **Migration / toggles** — `migrateLegacyMemory` is idempotent (bundle-with-concepts wins), atomic temp+rename writes, `MEMORY.md → .bak`. `JEO_MEMORY_LEGACY=1` forces the legacy doc; `JEO_NO_MEMORY=1` disables everything (wins over all). ✅

**Speed risks to watch (minor, not regressions):** (1) `selectWithinBudget` re-renders the whole selected set per candidate — O(n²) string build; negligible for tens of concepts, noticeable only for a hundreds-strong bundle. (2) `loadConcepts` walks + `stat`s every bundle file on each call (cache avoids re-parse, not the walk); repeated across many subagent spawns. (3) Concept count is unbounded on disk — injection is capped but the full walk/parse grows with bundle size over months. None block; candidates if a bundle ever grows large.

### Verification (this pass)
`bun run typecheck` clean. `bun test` on the touched/relevant files green: file-attachment, input-box, dev-link, team-run, team-schema, team-subagent → **70 pass / 0 fail**. README edits are docs-only (no code touched). gajae-code-analysis.md + .specify/ remain untracked.