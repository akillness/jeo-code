...
- **Release branch/package metadata** — prepared `release/v0.8.43` and bumped the npm package version from `0.8.42` to `0.8.43`; the release includes the already-verified detached lifecycle, monitor, cancellation, and TUI parallel-worker changes in this worktree.
...
### Added
- **Turn-scoped `monitor` tool** (`src/agent/monitor-tool.ts`, `src/agent/job-registry.ts`) — starts bounded stdout monitors through the existing job registry, with newline buffering, CRLF/EOF handling, one-shot or persistent delivery, timeout cleanup, strict native schema validation, TUI/plain-stream lifecycle rows, and an explicitly guard-neutral background-observation path. Monitor output never enters the user-steering inbox.
- **Detached lifecycle visibility** — detached `task` calls now carry stable `executor-N` identities through lifecycle events; the TUI keys detached workers independently, and plain logs retain the same identity instead of collapsing concurrent workers.

### Fixed
- **Detached dispatch and teardown correctness** (`src/agent/task-tool.ts`, `src/agent/engine.ts`, `src/agent/tools.ts`) — model-emitted `detached:"true"` now launches rather than silently blocking; the task path caps active detached runs at four; cancellation signals reach `bash` process groups and the mutation boundaries of `write`/`edit`/`mkdir`/`delete`. A detached fan-out request now fails explicitly rather than silently degrading to synchronous work.
- **A full-suite-only process-start assertion** (`test/notify-daemon-control.test.ts`) no longer assumes the Bun test worker is less than one minute old; it validates the OS timestamp against `process.uptime()`, eliminating a real timing-shaped false failure after long suite runs.

- **Anthropic rejects the `effort` parameter on a live-discovered model snapshot the version-based transport classifier did not expect to reject it** (`src/ai/providers/anthropic.ts`). Live-reproduced: a rate-limit fallback chain (subagent's own model rate-limited → "equivalent" fallback) landed on `claude-sonnet-4-5-20250929`, which classifies onto the `budget-effort` thinking transport by name/version (same as its `claude-sonnet-4-5` alias) but 400'd with "This model does not support the effort parameter" — the exact wording already known for Haiku 4.5, but on a model this codebase's classifier did not special-case. New `isEffortUnsupportedError` fail-safe (mirrors the existing deprecated-temperature/reasoning-artifact/fallback-unsupported retries in the same request loop) retries once with `output_config` dropped — `thinking`/`budget_tokens`, if any, survive; only the effort field itself is unsupported. Retry budget raised 4→5 attempts to fit the new flip alongside the three existing ones.
- **A `tasks` fan-out call sent as a JSON-STRINGIFIED array (correct content, wrong JSON type) was hard-rejected instead of recovered** (`src/agent/task-tool.ts`). Live-reproduced across two different models/providers (including a native-tool-calling-capable one): models repeatedly self-diagnosed "I'm passing tasks as a string instead of an actual array" turn after turn, never recovering, until the 5-consecutive-failure loop guard stopped them — a real, observed reliability gap in the flagship "fan out N concurrent subagents in one call" workflow. `createTaskTool`'s handler now transparently `JSON.parse`s a string `tasks` value when it parses to a real array, before falling through to the ordinary single-task path.
- **The `task`-tool-call preview box showed a bare "&lt;missing task&gt;" placeholder for EVERY fan-out/batch call**, well-formed or not (`src/tui/components/forge.ts`) — the preview only ever read a single `task`/`prompt`/`assignment` string, never the `tasks` array the concurrent path actually uses, so a user watching a real, successful 3-task fan-out saw zero useful preview of what was about to run. Now renders a numbered per-task summary (`1. …`, `2. …`, …) for both a real array and the JSON-stringified-array mistake above, title becomes `Task: <role> ×N`.
- **`taskToolProtocolLine`'s fan-out shape was compressed into an ambiguous one-line shorthand** (`{role, task|tasks[], context?}`) that plausibly contributed to the stringified-array mistake above — rewritten with two separate, concrete-example shapes (single vs fan-out) mirroring the `calc` tool's already-clear `{calculations:[{expression, prefix?, suffix?}]}` style, plus an explicit "MUST be a real JSON array... NEVER a JSON-stringified array, NEVER a single task string" warning.

### Verified
- Live (unmocked), multiple real `jeo --tmux` sessions with real Anthropic/Antigravity OAuth credentials: reproduced the original `<missing task>` + repeated-failure bug end to end; after the fixes, the SAME fan-out request correctly dispatched 3 concurrent executor subagents with a live "parallel · 3 running" TUI panel showing real-time per-task spinners and labels — confirming jeo's existing concurrent-monitoring TUI panel itself works correctly once the dispatch path is fixed.
- `test/anthropic-fallback.test.ts` +1 (effort-unsupported retry, exactly 2 fetch calls, `output_config` dropped while `thinking.budget_tokens` survives); `test/task-tool.test.ts` +3 (JSON-stringified `tasks` coercion, protocol-line wording, not-JSON/non-array fallthrough); `test/forge-status.test.ts` +4 (single-task unchanged, real array, JSON-stringified array, malformed fallthrough) — 82 pass / 0 fail across the 5 targeted files.
- `bun run typecheck` clean.
- Focused async coverage: detached lifecycle/cap/cancellation, monitor buffering/timeout, guard-neutral engine injection, native schema and slash guidance, TUI slots, and plain stream IDs.
- Isolated operational fixture: the actual `jeo --tmux` runtime, driven through a local OpenAI-compatible provider, rendered `parallel · 3 running` with distinct detached IDs; monitor start/line/done events appeared in the TUI; monitor data reached the model only under its non-instructional label; teardown left all delayed child-output files absent.
- `test/launch-telegram-remote.test.ts` now starts its notification-only harness with `--no-tools`, preventing an unrelated leaked LLM mock from mutating the shared test workspace.
- Full `bun test`: 3173 pass / 0 fail across 309 files.
...
### Added
- **`/route on save` / `/route off save` / bare `/route save`** (`src/commands/launch/route-slash.ts`) — persists `routing.enabled` to `~/.jeo/config.json` (mirrors `/model save`'s existing session-vs-persistent split), so routing preference now survives a restart without re-toggling every session. The bare form persists whatever is currently effective (an active session override, else the config's existing value); the `on`/`off save` forms also set the session-local override in the same call. All three carefully merge into the existing `routing` object rather than replacing it, so previously configured `tiers`/`confidenceThreshold`/`crossProviderPool` are never clobbered.

### Verified
- Live (unmocked), with real credentials and a real config copy: `/route on save` wrote `{"routing":{"enabled":true}}` to disk; a fresh `jeo --tmux` process launched against that same config dir immediately reported `routing: on (this session)` via `/route status` with zero re-toggling.
- `test/route-slash.test.ts` — 28 pass (was 22): 6 new tests cover `on save`/`off save`/bare `save`, config-field preservation (tiers/confidenceThreshold untouched), and confirming a plain `/route on` (no `save`) never touches disk.
- Full `bun test` — 3130 pass / 4 pre-existing unrelated flakes (browser screenshot/close timing, LSP definition timing, a perf-threshold test) across 307 files, all 4 confirmed passing in isolated re-runs.
- `bun run typecheck` — no errors.

### Fixed
- **Telegram daemon lock PID-reuse safety** (`src/agent/notify/daemon-control.ts`, gjc #2786 parity: "restore macOS daemon signaling (kill(2) + start-time recheck)") — `isPidAlive`'s bare `kill(pid, 0)` cannot distinguish a live daemon from an unrelated process that later reused the same pid after the daemon crashed; reproduced live (`isPidAlive(process.pid)` on an arbitrary unrelated process reports "alive"). New `isLockOwnerAlive` cross-checks the recorded `startedAt` against the OS-reported real process start time (`ps -o etime=`, parsed and diffed with a 10s tolerance) before trusting a lock's pid — used by `acquireDaemonLock`, `daemonStatus`, `startDaemon`, and (most importantly) `stopDaemon`, which previously could have sent a real `SIGTERM` to a completely unrelated process. Falls back to the old existence-only check whenever the OS lookup itself is unavailable (Windows, `ps` missing) — never a regression there.
- `test/notify-daemon-control.test.ts` — 22 pass (was 19): 3 new tests directly reproduce PID reuse (mismatched `startedAt` on a live pid reports not-alive; `stopDaemon` refuses to signal the mismatched pid and clears the lock instead, verified the unrelated live process survives).
- `test/daemon-command.test.ts` — fixed to use a real process start time instead of an arbitrary placeholder timestamp (the new stricter check correctly flagged the old fixture as a mismatch).
