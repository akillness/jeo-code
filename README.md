...
- **Parallel subagents, bounded async delegation, and monitors** — `task {tasks:["task 1","task 2"]}` runs independent assignments concurrently under a `MAX_FANOUT` bound (executor and read-only roles; results retain input order). `task {detached:true}` runs up to four independently cancellable subagents with stable IDs, lifecycle controls, and `irc` steering. `monitor {command, kind, description, timeout?, persistent?}` streams bounded stdout observations into the turn as explicitly non-instructional background data; TUI and plain logs keep every live worker distinct.
