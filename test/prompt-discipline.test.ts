...
test("WORKING_DISCIPLINE: don't-assume-disk + anti-hallucination + merged trust/verify line", () => {
  ...
  expect(WORKING_DISCIPLINE).toContain("Trust tool output as truth");
...
// Token budget: WORKING_DISCIPLINE is documented "<300 tokens". Use a coarse chars/4
// proxy and assert it (plus the separately-budgeted OUTPUT_DISCIPLINE) stays lean.
test("token budget: WORKING_DISCIPLINE stays under the ~300-token intent", () => {
  const approxTokens = (s: string) => Math.ceil(s.length / 4);
  expect(approxTokens(WORKING_DISCIPLINE)).toBeLessThan(300);
