...
<p align="center">
  A real <b>LLM coding agent</b> + <b>autopilot × autoresearch ratchet</b> + cross-plan ledger —
...
`jeoc` keeps the gajae-code philosophy — **small public surface, hardened runtime**. It is a
real LLM coding agent (`jeoc agent`) with provider/model config across Gemini, Anthropic, OpenAI
(+ a hermetic mock), plus an [autopilot](skills/autopilot/SKILL.md) build loop that **proves**
every change with a frozen evaluator (the [autoresearch](docs/04-autopilot-autoresearch.md) ratchet)
and a cross-plan ledger.
...
## `jeoc agent` — real LLM coding agent

A tool-calling turn loop (mirrors gjc's agent-core): `system + messages + tools → provider →
run tool calls → append results → repeat until done or max turns`. Tools: `bash`, `read_file`,
`write_file`, `list_dir`.
