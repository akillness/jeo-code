...
- **병렬 서브에이전트·제한된 비동기 위임과 모니터** — `task {tasks:["task 1","task 2"]}`는 독립적인 할당을 `MAX_FANOUT` 한도 안에서 동시에 실행하며(실행기·읽기 전용 역할 모두 지원, 결과 순서는 입력 순서를 유지), `task {detached:true}`는 안정적인 ID를 가진 독립 취소 가능 서브에이전트를 최대 네 개까지 실행하고 수명주기 제어와 `irc` 스티어링을 제공합니다. `monitor {command, kind, description, timeout?, persistent?}`는 제한된 stdout 관찰 결과를 명시적으로 지시가 아닌 백그라운드 데이터로 턴에 전달하며, TUI와 일반 로그는 실행 중인 워커를 각각 구분해 표시합니다.
