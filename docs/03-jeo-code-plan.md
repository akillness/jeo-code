...
- [x] **OAuth 인증 + 로컬 provider (v0.3.0)**: `jeoc auth`(login/status/logout, `~/.jeoc/auth.json` chmod 600) — OAuth access token을 `Authorization: Bearer`로 사용(Anthropic `anthropic-beta: oauth`, Gemini Bearer, OpenAI Bearer), API key보다 우선. 로컬 provider `ollama`(`/api/chat`, 키 불필요) + 기본 모델 `qwen2.5:0.5b`. `jeoc doctor`가 ollama 서버/모델 pull 점검. **실증**: 로컬 모델이 실제 `write_file` tool_call로 12바이트 파일 디스크 기록(오프라인 전체 루프). 29/29 테스트 통과. docs/09 추가.
